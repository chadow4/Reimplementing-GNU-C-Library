...
void checkDirectory(DIR * directory){
    ...
    while ( (directory_entry = readdir(directory)) != NULL ){
        if(stat(directory_entry->d_name,statsInfos) == 0){
            if(S_ISDIR(statsInfos->st_mode)){
                if(mini_strcmp(directory_entry->d_name, ".") != 0 && mini_strcmp(directory_entry->d_name, "..") != 0){
                    DIR *new_directory = opendir(directory_entry->d_name);
